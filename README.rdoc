...
For very special needs and requirements concerning query parsing (like weighted partial queries or more complex query constructs),
it is possible to override the default parser implementation through dependency injection. Just pass a :parser_klass option with
the parser class you want to be using to any of the following search methods:

- search
- find_similar_to
- facets (only uses the parser when the :class option is used)

The class should have exactly the same interface as XapianDb::QueryParser. Namely, the following conventions need to be followed:

- the signature of the constructor must be initialize(db), with db being a XapianDb::Database
- there must be an instance method parse(expression), which returns a Xapian::Query
- after parsing an expression, the parser must respond to :spelling_suggestion, preferrably returning get_corrected_query_string from an underlying Xapian::QueryParser

The easiest way to build your own parser is to use the XapianDb::QueryParser as your foundation and add your own parsing functionality
to the parse(expression) instance method.
