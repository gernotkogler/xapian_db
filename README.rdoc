...
You can add a type information to an attribute. As of now only the special types :date and :number are supported (and needed for range queries):
...
If you have a attribute of type :date defined, you can do date range searches:

  XapianDb::DocumentBlueprint.setup(Person) do |blueprint|
    blueprint.attribute :date_of_birth, :as => :date
  end

  result = XapianDb.search("date:2011-01-01..2011-12-31")

If you have a attribute of type :number defined, you can do number range searches:
